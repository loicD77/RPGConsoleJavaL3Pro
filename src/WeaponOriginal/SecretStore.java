...
public class SecretStore extends DungeonPiece {

    private List<Weapon> weapons;

    public SecretStore(String name, String description, int requiredLevel) {
        super(name, description, requiredLevel);
        // Initialiser la liste des armes disponibles dans le magasin secret
    ...
    public void enter(Player player) {
        ...
        visitSecretStore(player, scanner);
    }

    public void visitSecretStore(Player player, Scanner scanner) {
        boolean inStore = true;

        while (inStore) {
            // Afficher l'art ASCII du magasin secret avec le visage du joueur
            ...
            if (choice == weapons.size() + 1) {  // Quitter le magasin
                System.out.println("Vous quittez le magasin secret.");
                inStore = false;
                continue;
            }
    ...
    public void showWeapons() {
        System.out.println("Bienvenue au magasin secret !");
        int i = 1;
